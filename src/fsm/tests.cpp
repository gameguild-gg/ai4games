// File-based tests for FSM using doctest and the tests/ fixtures
...
static std::vector<std::pair<std::string, std::string>> findTestFiles()
{
  ...
  // Get the path to the executable to find tests relative to it
  fs::path exePath = fs::path(__FILE__).parent_path();

  std::vector<fs::path> candidates = {
      cwd / "tests",
      cwd / "out" / "fsm" / "tests", // Build output directory
      cwd / "fsm" / "tests",
      cwd / "src" / "fsm" / "tests",
      cwd / ".." / "tests",
      cwd / ".." / "fsm" / "tests",
...
TEST_CASE("FSM file-based tests")
{
  ...
  for (const auto &[inPath, outPath] : testFiles)
  {
    SUBCASE(fs::path(inPath).stem().string().c_str())
    {
      ...
      std::string actualOutput = FSM::runFSM(input);
